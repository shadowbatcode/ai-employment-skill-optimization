"""
主程序 v3 - 职位级时间序列预测（改进版）
- 优化速度：减少迭代次数
- 进度可视化：添加tqdm进度条
- 竞争关系：使用Related表构建竞争矩阵
- 多进程处理：分批次并行处理职位
"""
import numpy as np
import pandas as pd
...
def process_single_job(args):
    ...
    try:
        ...
        optimizer = ParameterOptimizerV3(model, train_data, ai_env)
        best_params, error = optimizer.optimize(maxiter=30, popsize=8)
        ...
        print(f"  [完成] {job_info['title']}: 误差={error:.6f}")
...
def main():
    """主函数（多进程版本）"""
    print("=" * 60)
    print("职位级生态协同进化模型 v3 - 多进程版")
    print("改进点：")
    print("  1. 优化速度（减少30%迭代次数）")
    print("  2. 进度可视化（实时显示优化进度）")
    print("  3. 竞争关系（基于O*NET相似度）")
    print("  4. 多进程处理（并行加速）")
    print("=" * 60)
...
def save_results(results):
    ...
    csv_file = 'job_parameters_v3.csv'
    ...
    # 保存合并的预测结果
    predictions_file = 'all_predictions_v3.csv'
