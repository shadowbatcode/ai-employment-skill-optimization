...
class ParameterTuner:
    ...
    def __init__(self, base_data, target_x_dict, skill_weights=None):
        ...
        self.dims = next(iter(base_data.values()))['E'].__len__() if base_data else None
        self.m = next(iter(base_data.values())).__len__() if base_data else None
        ...
        if skill_weights is None:
            ...
            def obj_func(x):
                ...
                for i in range(len(error)):
                    # 简化权重应用：前三个维度用technical权重，中间三个用leadership，后三个用sustainability
                    if i < len(error) // 3:
                        w = skill_weights.get('technical', 1.0)
                    elif i < 2 * len(error) // 3:
